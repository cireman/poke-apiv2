  ...
  return (
    <div>
      <article className="pokemonDetailpage">
        <img
          className="imgDetailsPokemon"
          src={pokeInfo?.sprites.other["official-artwork"].front_default}
        />

        <div className="containerDetails">
          <header
            className={`headerDetails br__${pokeInfo?.types[0].type.name}`}
          ></header>
          <div className={`pokemonId text__${pokeInfo?.types[0].type.name}`}>
            #{pokeInfo?.id}
          </div>
          ...
          <ul className="pokemonheight">
            <li>
              Peso<span>{pokeInfo?.weight}</span>
            </li>
            <li>
              Altura<span>{pokeInfo?.height}</span>
            </li>
          </ul>
          <div className="typesAndabilities">
            <ul>
              ...
              {pokeInfo?.types.map((type) => (
                <li
                  className={`features btn__${type.type.name}`}
                  key={type.type.url}
                >
                  {type.type.name}
                </li>
            ...
            <ul>
              ...
              {pokeInfo?.abilities.map((ability) => (
                <li className="features" key={ability.ability.url}>
          ...
          <div className="statsSection">
            <div className="statsHeader">
              ...
              <img className="pokeBolaBN" src="./pokebolafondo.png" />
            </div>
            ...
            <ul ref={ulRef} className="statsList">
              {pokeInfo?.stats.map((stat) => (
                <li key={stat.stat.url}>
                  ...
                  <div
                    className="statBaseBar"
                    style={{ display: animationName }}
                  >
                    <div
                      className="statBar"
          ...
          <div className="statsSection">
            ...
            <ul className="movementsUL">
              {pokeInfo?.moves.map((move) => (
                <li key={move.move.url} className="movements">
